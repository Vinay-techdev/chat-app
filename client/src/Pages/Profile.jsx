...
const Profile = () => {
  ...
  const [selectedImg, setSetlectedImg] = useState(null)
      ...
      <div className='w-5/6 max-w-2xl backdrop-blur-2xl text-gray-300 border-2 border-gray-600 flex items-center justify-between max-sm:flex-col-reverse rounded-lg'>
        ...
        <form onSubmit={(e) => handleSubmit(e)} className='flex flex-col gap-5 p-10 flex-1'>
          ...
          <label htmlFor="avatar" className='flex items-center gap-3 cursor-pointer'>
            <input onClick={(e) => setSetlectedImg(e.target.files[0])} id="avatar" type="file" accept="image/png, image/jpeg" hidden />
          ...
          <textarea onChange={(e) => setBio(e.target.value)} value={bio} rows={4} required placeholder='Write profile bio'
            className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500'> 
          </textarea>
