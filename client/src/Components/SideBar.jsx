...
const SideBar = () => {
  ...
  const filterUsers = input ? users.filter((user) => user.fullName.toLowerCase().includes(input.toLowerCase())) 
                            : users;
    ...
    >
      ...
      <div className="flex flex-col">
        {filterUsers.map((user, index) => (
          ...
          <div key={index} onClick={() => {setSelectedUser(user); setUnseenMessages(prev => ({...prev, [user._id]:0}))}} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-xs ${selectedUser?._id === user._id && "bg-[#282142]/50"}`} >
